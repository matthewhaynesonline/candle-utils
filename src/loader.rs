...
pub fn load_repo(model_id: &str, revision: &str) -> Result<ApiRepo> {
    let api = Api::new()
        .map_err(|e| candle_core::Error::Msg(format!("Couldn't create API. Error: {}", e)))?;
...
pub fn load_tokenizer(repo: &ApiRepo, tokenizer_file: Option<&Path>) -> Result<Tokenizer> {
    ...
    Tokenizer::from_file(tokenizer_filepath)
        .map_err(|e| candle_core::Error::Msg(format!("Couldn't load tokenizer. Error: {}", e)))
}
    ...
    C: ModelConfig,
{
    ...
    let config: C = serde_json::from_str(&config_str)
        .map_err(|e| candle_core::Error::Msg(format!("Couldn't parse config. Error: {}", e)))?;
